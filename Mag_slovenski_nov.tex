...
\Title{Preklop Hamiltonjana topološkega izolatorja z neredom preko točke faznega prehoda}			                %%% vnesite svoj naslov magistrskega dela
...
\cleardoublepage
\begin{center}
{\bf Preklop Hamiltonjana topološkega izolatorja z neredom preko točke faznega prehoda}\\[3mm]
{\sc  Izvle"cek}
\end{center}
\vspace{10mm}
V delu obravnavamo model enodimenzionalnega topološkega izolatorja z neredom. V takem sistemu lahko med topološkima fazama prehajamo tudi z višanjem jakosti nereda. Opazimo široko prevodno območje v okolici te točke faznega prehoda ter Andersonov lokalizacija/delokalizacija prehod točno na točki faznega prehoda.  Glavni del dela je posvečen proučevanju preklopov Hamiltonjana preko prej omenjene točke faznega prehoda. Med preklopom se pojavijo eksitacije v prevodnem pasu, katerih število je odvisno od hitrosti preklopa. Ugotovimo, da je pri počasnejsih preklopih odvisnost potenčna z nenavadnimi potencami. Potenčno skaliranje opazimo tudi pri najvišje zasedenih energijah v odvisnosti od hitrosti preklopa. Po podrobnejši analizi posameznih eksitacij opazimo, da posamezno preklopljeno stanje praviloma prehaja v le dve različni stanji v prevodnem pasu, kar razložimo s preslikavo na ekvivalenten XX model. \\[10mm]
...
\begin{center}
{\bf Disordered topological insulator Hamiltonian quench across a phase transition point}\\[3mm]
{\sc  Abstract}
\end{center}
\vspace{10mm}
We consider a model of a onedimensional topological insulator with disorder. In such a system a transition between topological phases is also possible by increasing the intensity of the disorder. We notice a wide conducting area around this phase transition point and the Anderson localization/delocalization transition exactly at the phase transition point. The main part of our work is devoted to the study of Hamiltonian quenches over the previously mentioned phase transition point. During the quench, exciatons in the conducting band appear with their number depending on the quench speed. For slower quenches, we observe a power law scaling of the number of excitations with respect to the quench speed with unusual exponents. Potential law is also seen in the dependence of the highest occupied energies in the conducting band of the quench speed. After a more detailed analysis of the individual excitations we notice that the quenched state generally only transitions into two different states in the conducting band, which is explained by mapping onto an equivalent XX model.\\[10mm]
...
Zanimiva lastnost Andersonove lokalizacije, ki bo za naše delo relevantna, je, da se v dovolj nizko dimenzionalnih rešetkah lokalizacija pojavi že pri poljubno majhni jakosti nereda (če je ta seveda prisoten). Tu bom predstavil intuitiven razlog za to lokalizacijo v eni dimenziji, konkretnejšo izpeljavo pa lahko bralec najde v \cite{anderson}.
...
\includegraphics[trim=0 0 0 38,clip,width=\linewidth]{Figures/SSHIPR2.pdf}
\end{subfigure}
...
Ureditveni parameter v prisotnosti nereda bo še vedno ovojno število, vendar je njen izračun tokrat nekoliko bolj zakompliciran.
...
\centering
\begin{subfigure}{.7\textwidth}
\includegraphics[trim=0 0 0 38,clip,width=\linewidth]{Figures/SSHIPRNered.pdf}
\end{subfigure}
...
Preklop delamo med dvema vrednostima jakosti nareda, tako da začnemo pri vrednosti $W_{\textup{zac}}<4$, končamo pa, po določenem času preklopa $T$, pri vrednosti $W_{\textup{kon}}>4$.
...
\end{figure}
Pravzaprav je smiselneje gledati število eksitacij v odvisnosti od hitrosti preklopa (Namreč, preklop $3.5 \rightarrow 4.5$ pri $T=10000$ je enako hiter kot preklop $2 \rightarrow 6$ pri $T=40000$), kar je prikazano na desnem grafu Slike \label{fig:Skaliranje}. Tu jasneje opazimo, da sta si preklopa na daljšem intervalu veliko bolj podobna v obnašanju - v primerjavi s preklopom na krajšem intervalu. 
\begin{figure}[H]
\centering
\begin{subfigure}{.99\textwidth}
...
Na Sliki \label{fig:SkoziCas} opazimo prej omenjeno hitro naraščanje števila eksitacij že takoj na začetku preklopa pri ozkem intervalu preklopa ter pri hitrih preklopih.
...
Poglejmo si še, kje natančneje se pojavijo te eksitacije. Na Sliki \ref{fig:Krajevne} je prikazana krajevna porazdelitev eksitacij med preklopom. Vidimo, da se eksitacije pojavljajo postopoma po nekih naključnih osnovnih celicah.
\begin{figure}[H]
\centering
\begin{subfigure}{.99\textwidth}
...
Na Sliki \ref{fig:Emax} si podrobneje pogledamo, kako je z ravnokar omenjenim višanjem energije končnih eksitacij s hitrostjo preklopa. Opazujemo, pri kateri energiji pade zasedenost stanj z $0.5$ na $0.25$, kar služi kot ocena za robno energijo, nad katero ni več veliko eksitacij. Če to narišemo v odvisnosti od hitrosti preklopa, opazimo, da je odvisnost spet potenčna, vendar je eksponent občutljiv na finost particije intervala energije in se giblje med $0.4$ in $0.6$. Takšna napaka eksponenta ni presenetljiva, saj je znano... TUKAJ BI DAL, ČE BI ŠLO, KAKŠNO REFERENCO NA TISTO DA SO EKSPONENTI PRI TEH STVAREH ZELO OBČUTLJIVI. ŠE NEKAJ: BI NA SPODNJI GRAF RAJE NARISAL ODVISNOSTI IN FITE ZA VEČ IZBIR BINOV? 
\begin{figure}[H]
\centering
...
TUKAJ SEM ZAČEL PISATI RAZLAGO ZA PREHOD V DVE STANJI. DOLOČENI KORAKI MI NISO 100\% JASNI, NAJBRŽ JE KAJ TUDI NAROBE.
...
Po drugi strani so pri $W>4$ v povprečju močnejše sklopitve znotraj iste osnovne celice torej $\{ J_{2i+1} \}$. Lastna stanja so tokrat singleti v isti osnovni celici.
